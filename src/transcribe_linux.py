...
def check_whisper_installation():
    ...
    try:
        # Tenta importar o módulo whisper diretamente
        import whisper
    ...
    try:
        ...
        print(f"❌ Erro ao verificar Whisper: {e}")
...
def get_audio_files():
    ...
    # Detecta se está rodando no Windows ou Linux
    base_path = os.getcwd()
    if os.name == 'nt':  # Windows
        input_dirs = [
            os.path.join(base_path, "INPUT"),
            os.path.join(base_path, "PaddleSpeech", "INPUT")
        ]
    else:  # Linux/WSL
        input_dirs = [
